...
process stage_default_signatures_proc {
    ...
    publishDir "${params.output_path}/temp", mode: 'copy', overwrite: true
    ...
    if [ -d "${workflow.projectDir}/signature_tables" ]; then
        ...
        echo "Staged repository signature_tables -> ${params.output_path}/temp/signature_tables"
    else
...
process stage_default_inputs_proc {
    ...
    if [ -d "${workflow.projectDir}/input_mutation_tables/${dataset}" ]; then
        ...
        echo "Staged repository input_mutation_tables/${dataset} -> ${params.output_path}/temp/input_tables/${dataset}"
    else
